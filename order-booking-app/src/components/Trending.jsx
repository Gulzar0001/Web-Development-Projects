...
import popData from "../utils/objectData/popData";

const Trending = () => {
  return (
<>
    <h1 className=" ml-3 font-bold text-4xl mb-4 ">Trending</h1>
    <div className="flex flex-wrap gap-2  mb-8 ml-3  ">
     

      {popData.map((item, index) => (
        
         <div key={index} className="w-1/6 mx-3 bg-gray-100 rounded-xl border p-3 relative">
         <div className="relative"> 
          <img
            src={item.img}
            alt={item.title}
            className="w-[95%]  object-contain rounded-md    "
